...
def step_impl(context, stack_name):
    ...
    try:
        ...
        if e.response["Error"]["Code"] == "ValidationError" and message.endswith(
            "does not exist"
        ):
            return
        else:
...
def step_impl(context, stack_name):
    ...
    try:
        ...
        if e.response["Error"]["Code"] == "ValidationError" and e.response["Error"][
            "Message"
...
@then('stack "{stack_name}" in "{region_name}" ' 'exists in "{desired_status}" state')
